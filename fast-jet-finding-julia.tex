...
benchmark machine is shown in Table \ref{tab:subtag}. Fastjet is somewhat faster
for filtering and trimming, while \JR\ is quite a lot faster for the mass drop
algorithm.

\begin{table}[ht]
    \begin{center}
        \begin{tabular}{l|ccc}
            \textbf{Method} & \textbf{\JR} & textbf{Fastjet} & Ratio\\
            \hline
        ...
        tagging operations in \JR\ and in Fastjet. The average is taken by
        performing these operations on all inclusive jets above 2GeV in all of
        our input sample files. Note that soft drop is not available in
        the Fastjet release.}
        \label{tab:subtag}
    \end{center}
\end{table}
